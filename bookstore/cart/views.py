...
# 前端发过来的数据:商品id 商品的数目 books_id books_count
# 涉及到数据的修改,使用post方式
def cart_add(request):
	...
	res = coon.hget(cart_key, books_id)
	if res is None:
		# 如果用户的购物车中没有添加过该商品,则添加数据
		res = count
	else:
		# 如果用户的购物车中已经添加过该商品, 则累计商品的数目
		res = int(res) + count
	...
	if res > books.stock:
		...
		coon.hset(cart_key, books_id, res)
...
def cart_count(request):
	...
	# res = coon.hlen(cart_key)  显示商品的条目数
	res = 0
...
def cart_update(request):
	...
	books = Books.objects.get_books_by_id(books_id=books_id)
	if books is None:
		return JsonResponse({'res':2, 'errmsg':'商品车不存在'})

	try:
		books_count = int(books_count)

	except Exception as e:
		return JsonResponse({'res':res, 'errmsg':'商品数目必须为数字'})
